...
function resumeAdventure() {
    ...
    if (gameState.currentAdventure.id === 'dark_forest') {
        // Restore the forest state
        if (gameState.currentAdventure.state) {
            window.forestState = gameState.currentAdventure.state;
        }
        resumeDarkForest(gameState.currentAdventure.currentScene || 'prologue');
    } else if (gameState.currentAdventure.id === 'dragon_cave') {
        // Restore the dragon cave state
        if (gameState.currentAdventure.state) {
            window.dragonState = gameState.currentAdventure.state;
        }
        resumeDragonCave(gameState.currentAdventure.currentScene || 'prologue');
    }
}
